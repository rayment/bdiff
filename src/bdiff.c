...
usage(char *argv)
{
	...
	fprintf(stderr,
	        "    -l      set number of lines to output, or < 1 for infinite\n");
	...
	while (1)
	{
		...
		if (dirty)
		{
			...
			if (hits < limit || limit <= 0)
				print_block(bufa, bufb, flags, block, color);
		}
		++block;
		if (feof(fa) || feof(fb))
			break;
     ...
     char **argv)
{
	...
	while ((c = getopt(argc, argv, "chl:v")) != -1)
	{
		switch (c)
		{
		case 'c':
	...
	return run(*(argv+optind), *(argv+optind+1), limit, color);
}
